...
export default function SignIn() {
	...
	const handleSignIn = async (values: z.infer<typeof FormSchema>) => {
		try {
			...
			// Redirect to next URL if available, or to home page with locale
			if (formattedNext) {
				router.push(formattedNext);
			} else {
				router.push(`/${locale}`);
			}
		} catch (error) {
		...
		<div className="w-full sm:w-[26rem] shadow sm:p-5  border dark:border-zinc-800 rounded-md">
			<div className="p-5 space-y-5">
				<div className="text-center space-y-3">
					...
					<p className="text-sm">{t('welcome-back')}</p>
				</div>
				...
				<Form {...form}>
					...
					<div className="text-center text-sm">
						<h1>
							{t('dont-have-account')}{' '}
							<Link
								href={`/${locale}/signup${formattedNext ? `?next=${formattedNext}` : ''}`}
